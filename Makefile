############################################################
# Makefile for the Nextlinux CLI, a simple command line interface to the
# Nextlinux Engine service. The rules, directives, and variables in this
...
DEV_IMAGE_REPO = nextlinux/nextlinux-cli-dev
...
.PHONY: build
build: Dockerfile nextlinux-ci venv ## Build dev Nextlinux CLI Docker image
...
.PHONY: lint
lint: venv nextlinux-ci ## Lint code (currently using flake8)
...
.PHONY: test-unit
test-unit: nextlinux-ci venv ## Run unit tests (tox)
	@$(ACTIVATE_VENV) && $(CI_CMD) test-unit

.PHONY: test-functional
test-functional: nextlinux-ci venv ## Run functional tests (tox)
	@$(ACTIVATE_VENV) && $(CI_CMD) test-functional tests/functional/tox.ini

.PHONY: setup-e2e-tests
setup-e2e-tests: nextlinux-ci venv ## Start kind cluster and set up end to end tests
	@$(MAKE) cluster-up
	@$(ACTIVATE_VENV) && $(CI_CMD) setup-e2e-tests "$(COMMIT_SHA)" "$(DEV_IMAGE_REPO)" "$(GIT_TAG)" "$(TEST_IMAGE_NAME)"

.PHONY: test-e2e
test-e2e: nextlinux-ci venv ## Run end to end tests (assuming cluster is running and set up has been run)
	@$(ACTIVATE_VENV) && $(CI_CMD) test-cli

.PHONY: setup-and-test-e2e
setup-and-test-e2e: nextlinux-ci venv ## Set up and run end to end tests
...
.PHONY: push-dev
push-dev: nextlinux-ci ## Push dev Nextlinux CLI Docker image to Docker Hub
	@$(CI_CMD) push-dev-image "$(COMMIT_SHA)" "$(DEV_IMAGE_REPO)" "$(GIT_BRANCH)" "$(TEST_IMAGE_NAME)"

.PHONY: push-rc
push-rc: nextlinux-ci ## (Not available outside of CI) Push RC Nextlinux CLI Docker image to Docker Hub
	@$(CI_CMD) push-rc-image "$(COMMIT_SHA)" "$(DEV_IMAGE_REPO)" "$(GIT_TAG)"

.PHONY: push-prod
push-prod: nextlinux-ci ## (Not available outside of CI) Push release Nextlinux CLI Docker image to Docker Hub
	@$(CI_CMD) push-prod-image-release "$(DEV_IMAGE_REPO)" "$(GIT_BRANCH)" "$(GIT_TAG)"

.PHONY: push-rebuild
push-rebuild: nextlinux-ci ## (Not available outside of CI) Rebuild and push prod Nextlinux CLI docker image to Docker Hub
...
ironbank-artifacts: nextlinux-ci ## (Not available outside of CI) Create and upload ironbank buildblob artifacts
	@$(CI_CMD) create-ironbank-artifacts nextlinux-cli "$(GIT_TAG)"

# Helper targets
####################
.PHONY: cluster-up
cluster-up: nextlinux-ci venv ## Stand up/start kind cluster
	@$(CI_CMD) install-cluster-deps "$(VENV)"
	@$(ACTIVATE_VENV) && $(CI_CMD) cluster-up "$(CLUSTER_NAME)" "$(CLUSTER_CONFIG)" "$(K8S_VERSION)"

.PHONY: cluster-down
cluster-down: nextlinux-ci venv ## Tear down/stop kind cluster
	@$(CI_CMD) install-cluster-deps "$(VENV)"
