...
def setup_config(cli_opts):
    ret = {
        ...
        "hub-url": "https://hub.nextlinux.io/",
        "api-version": None,
        "ssl_verify": True,
        "jsonmode": False,
        "debug": False,
    ...
    for e in [
        ...
        "NEXTLINUX_CLI_PASS",
        "NEXTLINUX_CLI_URL",
        "NEXTLINUX_CLI_HUB_URL",
        "NEXTLINUX_CLI_API_VERSION",
        "NEXTLINUX_CLI_SSL_VERIFY",
        "NEXTLINUX_CLI_JSON",
        "NEXTLINUX_CLI_DEBUG",
        "NEXTLINUX_CLI_ACCOUNT",
        "NEXTLINUX_CLI_CONFIG",
    ]:
        if e in os.environ:
    ...
    try:
        if "NEXTLINUX_CLI_CONFIG" in settings:
            credential_file = settings["NEXTLINUX_CLI_CONFIG"]
        else:
            home = os.path.expanduser("~")
            credential_file = os.path.join(home, ".nextlinux", "credentials.yaml")
    ...
    if cli_opts["config"]:
        settings["NEXTLINUX_CLI_CONFIG"] = cli_opts["config"]
    if cli_opts["u"]:
        settings["NEXTLINUX_CLI_USER"] = cli_opts["u"]

    if cli_opts["p"]:
        settings["NEXTLINUX_CLI_PASS"] = cli_opts["p"]

    if cli_opts["url"]:
        settings["NEXTLINUX_CLI_URL"] = cli_opts["url"]

    if cli_opts["hub-url"]:
        settings["NEXTLINUX_CLI_HUB_URL"] = cli_opts["hub-url"]

    if cli_opts["api-version"]:
        settings["NEXTLINUX_CLI_API_VERSION"] = cli_opts["api-version"]

    if cli_opts["insecure"]:
        settings["NEXTLINUX_CLI_SSL_VERIFY"] = "n"
    ...
    if cli_opts.get("as_account") is not None:
        settings["NEXTLINUX_CLI_ACCOUNT"] = cli_opts["as_account"]

    if "NEXTLINUX_CLI_CONFIG" in settings:
        ret["config"] = settings["NEXTLINUX_CLI_CONFIG"]
    if "NEXTLINUX_CLI_USER" in settings:
        ret["user"] = settings["NEXTLINUX_CLI_USER"]
    if "NEXTLINUX_CLI_PASS" in settings:
        ret["pass"] = settings["NEXTLINUX_CLI_PASS"]
    if "NEXTLINUX_CLI_URL" in settings:
        ret["url"] = settings["NEXTLINUX_CLI_URL"]
    if "NEXTLINUX_CLI_HUB_URL" in settings:
        ret["hub-url"] = settings["NEXTLINUX_CLI_HUB_URL"]

    if "NEXTLINUX_CLI_API_VERSION" in settings:
        ret["api-version"] = settings["NEXTLINUX_CLI_API_VERSION"]
    if "NEXTLINUX_CLI_SSL_VERIFY" in settings:
        if settings["NEXTLINUX_CLI_SSL_VERIFY"].lower() == "n":
            ret["ssl_verify"] = False
    if "NEXTLINUX_CLI_JSON" in settings:
        if settings["NEXTLINUX_CLI_JSON"].lower() == "y":
            ret["jsonmode"] = True
    if "NEXTLINUX_CLI_DEBUG" in settings:
        if settings["NEXTLINUX_CLI_DEBUG"].lower() == "y":
            ret["debug"] = True
    if "NEXTLINUX_CLI_ACCOUNT" in settings:
        ret["as_account"] = settings["NEXTLINUX_CLI_ACCOUNT"]

    return ret


def doexit(ecode):
    if not os.environ.get("NEXTLINUX_CLI_NO_FDS_CLEANUP"):
        try:
            sys.stdout.close()
        except Exception:
            pass
...
def check_access(config):
    # test the endpoint
    try:
        rc = nextlinuxcli.clients.apiexternal.get_base_routes(config)
        if not rc["success"]:
            raise Exception(json.dumps(rc["error"], sort_keys=True))
    except Exception as err:
        if config["debug"]:
            raise Exception(
                "could not access nextlinux service (user="
                + str(config["user"])
                + " url="
                + str(config["url"])
                + "): "
...
def discover_inputimage(config, input_string):
    ...
    ret_type = "tag"
    try:
        ret = nextlinuxcli.clients.apiexternal.get_image(config, tag=input_string)
        if ret["success"]:
