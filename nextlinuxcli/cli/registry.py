...
def registry(ctx_config):
    ...
    try:
        ...
        print(nextlinuxcli.cli.utils.format_error_output(config, "registry", {}, err))
        sys.exit(2)
...
def add(
    ...
    try:
        ...
        ret = nextlinuxcli.clients.apiexternal.add_registry(
            config,
            registry=registry,
        ...
        print(nextlinuxcli.cli.utils.format_error_output(config, "registry_add", {}, err))
        if not ecode:
            ecode = 2
...
def upd(
    ...
    try:
        ...
        ret = nextlinuxcli.clients.apiexternal.update_registry(
            config,
            registry=registry,
            registry_user=registry_user,
            registry_pass=registry_pass,
        ...
        print(
            nextlinuxcli.cli.utils.format_error_output(config, "registry_update", {}, err)
        )
        if not ecode:
            ecode = 2
    nextlinuxcli.cli.utils.doexit(ecode)
...
def delete(registry):
    ...
    try:
        ret = nextlinuxcli.clients.apiexternal.delete_registry(config, registry)
        ecode = nextlinuxcli.cli.utils.get_ecode(ret)
        if ret["success"]:
            print(
                nextlinuxcli.cli.utils.format_output(
        ...
        print(
            nextlinuxcli.cli.utils.format_error_output(config, "registry_delete", {}, err)
        )
        if not ecode:
            ecode = 2
    nextlinuxcli.cli.utils.doexit(ecode)
...
def registrylist():
    ...
    try:
        ret = nextlinuxcli.clients.apiexternal.get_registry(config)
        ecode = nextlinuxcli.cli.utils.get_ecode(ret)
        if ret["success"]:
            print(
                nextlinuxcli.cli.utils.format_output(
        ...
        print(
            nextlinuxcli.cli.utils.format_error_output(config, "registry_list", {}, err)
        )
        if not ecode:
            ecode = 2
    nextlinuxcli.cli.utils.doexit(ecode)
...
def get(registry):
    ...
    try:
        ret = nextlinuxcli.clients.apiexternal.get_registry(config, registry=registry)
        ecode = nextlinuxcli.cli.utils.get_ecode(ret)
        if ret["success"]:
            print(
                nextlinuxcli.cli.utils.format_output(
        ...
        print(nextlinuxcli.cli.utils.format_error_output(config, "registry_get", {}, err))
        if not ecode:
            ecode = 2
    nextlinuxcli.cli.utils.doexit(ecode)
