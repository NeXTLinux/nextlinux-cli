...
import nextlinuxcli.clients.apiexternal
import nextlinuxcli.cli.utils
...
def system(ctx_config, ctx):
    ...
    if ctx.invoked_subcommand not in ["wait"]:
        try:
            nextlinuxcli.cli.utils.check_access(config)
        except Exception as err:
            print(nextlinuxcli.cli.utils.format_error_output(config, "system", {}, err))
            sys.exit(2)


@system.command(name="status", short_help="Check current nextlinux-engine system status")
def status():
    ecode = 0

    try:
        ret = nextlinuxcli.clients.apiexternal.system_status(config)
        ecode = nextlinuxcli.cli.utils.get_ecode(ret)
        if ret["success"]:
            print(
                nextlinuxcli.cli.utils.format_output(
        ...
        print(
            nextlinuxcli.cli.utils.format_error_output(config, "system_status", {}, err)
        )
        if not ecode:
            ecode = 2

    nextlinuxcli.cli.utils.doexit(ecode)


@system.command(
    name="errorcodes",
    short_help="Describe available nextlinux system error code names and descriptions",
)
def describe_errorcodes():
    ecode = 0

    try:
        ret = nextlinuxcli.clients.apiexternal.describe_error_codes(config)
        ecode = nextlinuxcli.cli.utils.get_ecode(ret)
        if ret["success"]:
            print(
                nextlinuxcli.cli.utils.format_output(
        ...
        print(
            nextlinuxcli.cli.utils.format_error_output(
...
@system.command(
    name="wait",
    short_help="Blocking operation that will return when nextlinux-engine is available and ready",
)
@click.option(
    "--timeout",
    type=float,
...
@click.option(
    ...
    help='Wait for the specified CSV list of nextlinux-engine services to have at least one service reporting as available (default="catalog,apiext,policy_engine,simplequeue,analyzer")',
)
def wait(timeout, interval, feedsready, servicesready):
    ...
    try:
        sys.stderr.write(
            "Starting checks to wait for nextlinux-engine to be available timeout={} interval={}\n".format(
                timeout, interval
            )
        )
        ts = time.time()
        while timeout < 0 or time.time() - ts < timeout:
            _logger.debug(
                "Checking API availability for nextlinux-engine URL (%s)", config["url"]
            )
            # FIXME this can still break when formatting
            sys.stderr.write(
                "API availability: Checking nextlinux-engine URL ({})...\n".format(
                    config["url"]
                )
            )
            try:
        ...
        while timeout < 0 or time.time() - ts < timeout:
            ...
            _logger.debug(
                "Checking service set availability for nextlinux-engine URL (%s)",
                config["url"],
            )
            # FIXME this can still break when formatting
            sys.stderr.write(
            ...
            try:
                ...
                if ret["success"]:
                    ...
                    raise Exception(
                        "service responded with 401 Unauthorized - please check nextlinux-engine credentials and try again"
                    )
            except Exception as err:
        ...
        if feedsready:
            ...
            while timeout < 0 or time.time() - ts < timeout:
                _logger.debug(
                    "Checking feed sync status for nextlinux-engine URL (%s)",
                    config["url"],
                )
                ...
                try:
                    ret = nextlinuxcli.clients.apiexternal.system_feeds_list(config)
                    if ret["success"]:
        ...
        print(nextlinuxcli.cli.utils.format_error_output(config, "system_wait", {}, err))
        if not ecode:
            ecode = 2
...
@system.command(
    name="del", short_help="Delete a non-active service from nextlinux-engine"
)
...
def delete(host_id, servicename):
    ...
    try:
        ret = nextlinuxcli.clients.apiexternal.delete_system_service(
...
def feedsync(flush):
    ...
    try:
        ...
        if "y" == answer.lower():
            ret = nextlinuxcli.clients.apiexternal.system_feeds_sync(config, flush)
            ecode = nextlinuxcli.cli.utils.get_ecode(ret)
...
def toggle_enabled(feed, group=None, enable=None, disable=None):
    ...
    try:
        ...
        if group:
            ret = nextlinuxcli.clients.apiexternal.system_feed_group_enable_toggle(
                config, feed, group, enabled=enabled
            )
            ecode = nextlinuxcli.cli.utils.get_ecode(ret)
        else:
            ret = nextlinuxcli.clients.apiexternal.system_feed_enable_toggle(
...
def delete_data(feed, group=None):
    ...
    try:
        if group:
            ret = nextlinuxcli.clients.apiexternal.system_feed_group_delete(
                config, feed, group
            )
            ecode = nextlinuxcli.cli.utils.get_ecode(ret)
        else:
            ret = nextlinuxcli.clients.apiexternal.system_feed_delete(config, feed)
            ecode = nextlinuxcli.cli.utils.get_ecode(ret)
...
def test_webhook(webhook_type, ntype):
    """
    Call the Test Webhook Endpoint on Nextlinux Engine
    ...
    try:
        ...
        ret = nextlinuxcli.clients.apiexternal.test_webhook(config, webhook_type, ntype)
        ecode = nextlinuxcli.cli.utils.get_ecode(ret)
        if ret["success"]:
            print(
                nextlinuxcli.cli.utils.format_output(
        ...
        print(nextlinuxcli.cli.utils.format_error_output(config, "test_webhook", {}, err))
        if not ecode:
            ecode = 2
