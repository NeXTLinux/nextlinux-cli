...
def evaluate(ctx_config):
    ...
    try:
        ...
        print(nextlinuxcli.cli.utils.format_error_output(config, "evaluate", {}, err))
        sys.exit(2)
...
def check(input_image, show_history, detail, tag, policy):
    ...
    try:
        ...
        if imageDigest:
            ...
            ret = nextlinuxcli.clients.apiexternal.check_eval(
                config,
            ...
            if ret["success"]:
                ...
                ecode = nextlinuxcli.cli.utils.get_eval_ecode(
                    ret["payload"], nextlinuxcli.cli.utils.unquote_plus(imageDigest)
                )
            else:
                raise Exception(json.dumps(ret["error"], indent=4))
        else:
            raise Exception("could not get image record from nextlinux")

    except Exception as err:
        print(
            nextlinuxcli.cli.utils.format_error_output(config, "evaluate_check", {}, err)
        )
        if not ecode:
            ecode = 2
