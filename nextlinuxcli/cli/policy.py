...
import nextlinuxcli.clients.apiexternal, nextlinuxcli.clients.hub
import nextlinuxcli.cli
...
def policy(ctx_config, ctx):
    ...
    if ctx.invoked_subcommand not in ["hub"]:
        try:
            ...
            print(nextlinuxcli.cli.utils.format_error_output(config, "policy", {}, err))
            sys.exit(2)
...
@click.argument(
    ...
    metavar="<Nextlinux Policy Bundle File>",
)
def add(input_policy):
    ...
    try:
        ...
        ret = nextlinuxcli.clients.apiexternal.add_policy(
        ...
        print(nextlinuxcli.cli.utils.format_error_output(config, "policy_add", {}, err))
        if not ecode:
            ecode = 2
...
def get(policyid, detail):
    ...
    try:
        ret = nextlinuxcli.clients.apiexternal.get_policy(
        ...
        print(nextlinuxcli.cli.utils.format_error_output(config, "policy_get", {}, err))
        if not ecode:
            ecode = 2
...
def policylist():
    ...
    try:
        ret = nextlinuxcli.clients.apiexternal.get_policies(config, detail=False)
        ecode = nextlinuxcli.cli.utils.get_ecode(ret)
        if ret["success"]:
            print(
                nextlinuxcli.cli.utils.format_output(
        ...
        print(nextlinuxcli.cli.utils.format_error_output(config, "policy_list", {}, err))
        if not ecode:
            ecode = 2
...
def activate(policyid):
    ...
    try:
        ...
        ret = nextlinuxcli.clients.apiexternal.update_policy(
        ...
        print(
            nextlinuxcli.cli.utils.format_error_output(config, "policy_activate", {}, err)
        )
        if not ecode:
            ecode = 2
...
def delete(policyid):
    ...
    try:
        ret = nextlinuxcli.clients.apiexternal.delete_policy(config, policyId=policyid)
        ecode = nextlinuxcli.cli.utils.get_ecode(ret)
        if ret["success"]:
            print(
                nextlinuxcli.cli.utils.format_output(
        ...
        print(
            nextlinuxcli.cli.utils.format_error_output(config, "policy_delete", {}, err)
        )
        if not ecode:
            ecode = 2
...
def describe(all=False, gate=None, trigger=None):
    ecode = 0
    try:
        ret = nextlinuxcli.clients.apiexternal.describe_policy_spec(config)
        ...
        print(
            nextlinuxcli.cli.utils.format_error_output(config, "describe_policy", {}, err)
        )
        if not ecode:
            ecode = 2
...
@policy.group(name="hub", short_help="Nextlinux Hub Operations")
@click.pass_context
def hub(ctx):
...
def hublist():
    ...
    try:
        ret = nextlinuxcli.clients.hub.get_policies(config)
        if ret["success"]:
            print(
                nextlinuxcli.cli.utils.format_output(
        ...
        print(
            nextlinuxcli.cli.utils.format_error_output(config, "policy_hub_list", {}, err)
        )
        if not ecode:
            ecode = 2
...
def hubget(bundlename):
    ...
    try:
        ret = nextlinuxcli.clients.hub.get_policy(config, bundlename)
        if ret["success"]:
            print(
                nextlinuxcli.cli.utils.format_output(
        ...
        print(
            nextlinuxcli.cli.utils.format_error_output(config, "policy_hub_get", {}, err)
        )
        if not ecode:
            ecode = 2
...
def hubinstall(bundlename, target_id, force):
    ...
    try:
        ret = nextlinuxcli.clients.hub.install_policy(
