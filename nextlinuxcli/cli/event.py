...
def event(ctx_config):
    ...
    try:
        ...
        print(nextlinuxcli.cli.utils.format_error_output(config, "event", {}, err))
        sys.exit(2)
...
def list(
    ...
    try:
        ...
        ret = nextlinuxcli.clients.apiexternal.list_events(
            config,
            since=since,
            before=before,
            level=level,
        ...
        print(nextlinuxcli.cli.utils.format_error_output(config, "event_list", {}, err))
        if not ecode:
            ecode = 2
...
def get(event_id):
    ...
    try:
        ret = nextlinuxcli.clients.apiexternal.get_event(config, event_id=event_id)
        ecode = nextlinuxcli.cli.utils.get_ecode(ret)
        if ret["success"]:
            print(
                nextlinuxcli.cli.utils.format_output(
        ...
        print(nextlinuxcli.cli.utils.format_error_output(config, "event_get", {}, err))
        if not ecode:
            ecode = 2
...
def delete(since=None, before=None, dontask=False, event_id=None, all=False):
    ...
    try:
        if event_id:
            ret = nextlinuxcli.clients.apiexternal.delete_event(config, event_id=event_id)
        else:
            if all:
                if dontask:
                    answer = "y"
            ...
            if "y" == answer.lower():
                ret = nextlinuxcli.clients.apiexternal.delete_events(
        ...
        print(nextlinuxcli.cli.utils.format_error_output(config, "event_delete", {}, err))
        if not ecode:
            ecode = 2
