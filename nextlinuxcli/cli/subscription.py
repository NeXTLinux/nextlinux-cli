...
def subscription(ctx_config):
    ...
    try:
        ...
        print(nextlinuxcli.cli.utils.format_error_output(config, "subscription", {}, err))
        sys.exit(2)
...
def activate(subscription_type, subscription_key):
    ...
    try:
        ret = nextlinuxcli.clients.apiexternal.activate_subscription(
...
def deactivate(subscription_type, subscription_key):
    ...
    try:
        ret = nextlinuxcli.clients.apiexternal.deactivate_subscription(
            config, subscription_type, subscription_key
        )
        ecode = nextlinuxcli.cli.utils.get_ecode(ret)
        if ret["success"]:
...
def list_subscriptions(full):
    ecode = 0
    try:
        ret = nextlinuxcli.clients.apiexternal.get_subscription(config)
        ecode = nextlinuxcli.cli.utils.get_ecode(ret)
        if ret["success"]:
            print(
                nextlinuxcli.cli.utils.format_output(
...
def get_subscription_by_id(subscription_id):
    return_code = 0
    try:
        ret = nextlinuxcli.clients.apiexternal.get_subscription_by_id(
            config, subscription_id
        )
        return_code = nextlinuxcli.cli.utils.get_ecode(ret)
        if ret["success"]:
            print(
                nextlinuxcli.cli.utils.format_output(
    ...
    nextlinuxcli.cli.utils.doexit(return_code)
...
def delete_subscription_by_id(subscription_id):
    return_code = 0
    try:
        ret = nextlinuxcli.clients.apiexternal.delete_subscription_by_id(
            config, subscription_id
        )
        return_code = nextlinuxcli.cli.utils.get_ecode(ret)
        if ret["success"]:
