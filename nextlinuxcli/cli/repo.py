...
def repo(ctx_config):
    ...
    try:
        ...
        print(nextlinuxcli.cli.utils.format_error_output(config, "repo", {}, err))
        sys.exit(2)
...
def add(input_repo, noautosubscribe, lookuptag, dryrun):
    ...
    image_info = nextlinuxcli.cli.utils.parse_dockerimage_string(input_repo)
    input_repo = image_info["registry"] + "/" + image_info["repo"]

    try:
        ret = nextlinuxcli.clients.apiexternal.add_repo(
            config,
            input_repo,
        ...
        response_code = nextlinuxcli.cli.utils.get_ecode(ret)
        if ret["success"]:
            print(
        ...
        print(nextlinuxcli.cli.utils.format_error_output(config, "repo_add", {}, err))
        if not response_code:
            response_code = 2

    nextlinuxcli.cli.utils.doexit(response_code)
...
def listrepos():
    ...
    try:
        ret = nextlinuxcli.clients.apiexternal.get_repo(config)
        ...
        print(nextlinuxcli.cli.utils.format_error_output(config, "repo_list", {}, err))
        if not ecode:
            ecode = 2
...
def get(input_repo):
    ...
    try:
        ret = nextlinuxcli.clients.apiexternal.get_repo(config, input_repo=input_repo)
        if ret:
            ecode = nextlinuxcli.cli.utils.get_ecode(ret)
            if ret["success"]:
                print(
        ...
        print(nextlinuxcli.cli.utils.format_error_output(config, "repo_get", {}, err))
        if not ecode:
            ecode = 2
...
def delete(input_repo):
    ...
    try:
        ret = nextlinuxcli.clients.apiexternal.delete_repo(config, input_repo)
        ecode = nextlinuxcli.cli.utils.get_ecode(ret)
        if ret:
            if ret["success"]:
                print(
        ...
        print(nextlinuxcli.cli.utils.format_error_output(config, "repo_delete", {}, err))
        if not ecode:
            ecode = 2
...
def unwatch(input_repo):
    ...
    try:
        ret = nextlinuxcli.clients.apiexternal.unwatch_repo(config, input_repo)
        ecode = nextlinuxcli.cli.utils.get_ecode(ret)
        if ret:
            if ret["success"]:
                print(
        ...
        print(nextlinuxcli.cli.utils.format_error_output(config, "repo_unwatch", {}, err))
        if not ecode:
            ecode = 2
...
def watch(input_repo):
    ...
    try:
        ret = nextlinuxcli.clients.apiexternal.watch_repo(config, input_repo)
        ecode = nextlinuxcli.cli.utils.get_ecode(ret)
        if ret:
            if ret["success"]:
                print(
        ...
        print(nextlinuxcli.cli.utils.format_error_output(config, "repo_watch", {}, err))
        if not ecode:
            ecode = 2
