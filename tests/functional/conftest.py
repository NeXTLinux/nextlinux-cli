...
def pytest_sessionstart(session):
    ...
    image = os.environ.get("PYTEST_CONTAINER", "nextlinux/inline-scan:latest")
...
def pytest_report_header(config):
    ...
    try:
        ...
        msg = ["Docker: Nextlinux inline_scan container not running yet"]
    except DockerException as e:
        logger.exception("Unable to connect to a docker socket")
        msg = ["Nextlinux Version: Unable to connect to a docker socket"]
        msg.append("Error: %s" % str(e))
        return msg

    labels = metadata["Config"]["Labels"]
    version = labels.get("version", "unknown")
    commit = labels.get("nextlinux_commit", "unknown")

    msg.extend(["Nextlinux Version: %s" % version, "Nextlinux Commit: %s" % commit])
    return msg
...
def start_container(client, image, name, environment, ports, detach=True):
    ...
    while time.time() - start < 70:
        out, err, code = call(
            ["nextlinux-cli", "--u", "admin", "--p", "foobar", "system", "status"],
        )
        if code == 0:
    ...
    # If 70 seconds passed and nextlinux-cli wasn't able to determine an OK
    # status from nextlinux-engine then failure needs to be raised with as much
...
def _call(command, **kw):
    if command[0] != "nextlinux-cli":
        command.insert(0, "nextlinux-cli")
    return call(
        command, env={"NEXTLINUX_CLI_USER": "admin", "NEXTLINUX_CLI_PASS": "foobar"}, **kw
    )
