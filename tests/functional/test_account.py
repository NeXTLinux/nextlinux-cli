...
def test_unauthorized(sub_command):
    out, err, code = call(["nextlinux-cli", "account", sub_command])
...
class TesttList:
    def test_is_authorized(self, admin_call):
        ...
        assert "AccountEmail: admin@mynextlinux" in out
    ...
    def test_is_authorized_json(self, admin_call):
        ...
        user = loaded["user"]
        assert account["email"] == "admin@mynextlinux"
...
class TestWhoami:
    def test_is_authorized(self, admin_call):
        ...
        # remove the TZ
        assert out[1][:-1] == ["admin", "admin@mynextlinux", "admin", "enabled"]
    ...
    def test_is_authorized_json(self, admin_call):
        ...
        assert loaded["email"] == "admin@mynextlinux"
